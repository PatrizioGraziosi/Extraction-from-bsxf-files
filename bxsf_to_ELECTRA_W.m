...
function bxsf_to_ELECTRA_W(fileName,material_name,alat,reduce_flag,interpolation_factor) %codegen
...
for id_band = 1:num_of_bands
    ...
    function [ Ek_i, kx_matrix_i, ky_matrix_i, kz_matrix_i ] = ...
        ...
        parfor i = 1:n_bands
            ...
            Ek_temp = Ek(:,:,:,i);
            Ek_m_temp = griddata(kx_matrix,ky_matrix,kz_matrix,Ek_temp,Kx_interp,Ky_interp,Kz_interp,'nearest'); % it was natural

            % Ek_m_temp = 0*Kx_interp ;
            % for ix = 1:nk_new_x
            %     for iy = 1:nk_new_y
            %         Ek_m_temp(ix,iy,:) = interp1(kz_matrix(ix))
            %     end
            % end
